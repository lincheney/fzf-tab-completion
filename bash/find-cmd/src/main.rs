...
impl<'a> Match<'a> {
    pub fn offset(self, offset: usize) -> Match<'a> {
        Match{ start: self.start+offset, end: self.end+offset, found: self.found, tokens: self.tokens }
    }
}
...
fn parse_dq_string<'a>(line: &'a str, point: usize) -> Match<'a> {
    ...
    return Match{start: 0, end: i, found: false, tokens: vec![line]};
}
...
fn parse_line<'a>(line: &'a str, point: usize, end: Option<&Regex>) -> Match<'a> {
    ...
    while i < line.len() {
        ...
        for &(open, close) in [
            ...
            if line[i..].starts_with(open) {
                ...
                if m.found { return m.offset(i); }
                if i == point {
                    return Match{ start: i, end: i+m.end, found: true, tokens: m.tokens };
                }
                i += m.end;
                if i+1 < line.len() { i += 1; }
        ...
        if start == Some(i) {
            ...
            if let Some(m) = ENV_VAR_RE.find(&line[i..]) {
                ...
                if i >= point || (m.found && m.start == 0) {
                    ...
                    let end = m.end + i;
                    return Match{ start: start, end: end, found: true, tokens: vec![&line[start..end]] };
                }
                if m.found { return m.offset(i); }
                i += m.end;
                start = None;
    ...
    Match{start: start, end: i, found: 0 < point && point <= i, tokens: tokens}
}
...
mod test {
    ...
    macro_rules! assert_parse_line {
        ($left:expr, $right:expr, $expected:expr, $array:expr) => {
    ...
    fn test_parse_line() {
        ...
        assert_parse_line!("echo", " '12(3", "echo '12(3", vec!["echo", "'12(3"]);
        assert_parse_line!("echo", " $'12\\'3'", "echo $'12\\'3'", vec!["echo", "$'12\\'3'"]);
        assert_parse_line!("echo", " '1\\'2\\\\(3'", "echo '1\\'2\\\\(3'", vec!["echo", "'1\\'2\\\\(3'"]);
        assert_parse_line!("`cat`; echo", "", "echo", vec!["echo"]);

        assert_parse_line!("[[ echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("case echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("do echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("done echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("elif echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("else echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("esac echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("fi echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("for echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("function echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("if echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("in echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("select echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("then echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("time echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("until echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("while echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("whil", "e echo 123", "while", vec!["while"]);

        assert_parse_line!("echo", " 123 ; echo 456", "echo 123 ", vec!["echo", "123"]);
        assert_parse_line!("echo 123 ; echo", " 456", "echo 456", vec!["echo", "456"]);
        assert_parse_line!("echo", " 123 \n echo 456", "echo 123 ", vec!["echo", "123"]);
        assert_parse_line!("echo 123 \n echo", " 456", "echo 456", vec!["echo", "456"]);
        assert_parse_line!("echo", " 123 && echo 456", "echo 123 ", vec!["echo", "123"]);
        assert_parse_line!("echo 123 && echo", " 456", "echo 456", vec!["echo", "456"]);
        assert_parse_line!("echo", " 123 || echo 456", "echo 123 ", vec!["echo", "123"]);
        assert_parse_line!("echo 123 || echo", " 456", "echo 456", vec!["echo", "456"]);
        ...
        assert_parse_line!("echo ${var", "}", "echo ${var}", vec!["echo", "${var}"]);
        assert_parse_line!("echo ${var", "", "echo ${var", vec!["echo", "${var"]);
        assert_parse_line!("echo $(cat) ", "123", "echo $(cat) 123", vec!["echo", "$(cat)", "123"]);
        assert_parse_line!("echo $(ca", "t) 123", "cat", vec!["cat"]);
        assert_parse_line!("echo $(", "cat) 123", "cat", vec!["cat"]);

        assert_parse_line!("KEY=VALUE echo", " 123", "echo 123", vec!["echo", "123"]);
        assert_parse_line!("KE", "Y=VALUE echo 123", "KEY=VALUE", vec!["KEY=VALUE"]);
        assert_parse_line!("KEY=VAL", "UE echo 123", "KEY=VALUE", vec!["KEY=VALUE"]);
        assert_parse_line!("KEY=VA$(ca", "t) echo 123", "cat", vec!["cat"]);
        assert_parse_line!("KEY=VALUE XYZ=", "STUFF echo 123", "XYZ=STUFF", vec!["XYZ=STUFF"]);
        assert_parse_line!("KEY=VALUE XYZ=$(ca", "t) echo 123", "cat", vec!["cat"]);
        assert_parse_line!("KEY=VALUE XYZ=$(ca", "t echo 123", "cat echo 123", vec!["cat", "echo", "123"]);
    }
}
